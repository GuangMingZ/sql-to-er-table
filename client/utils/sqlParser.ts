...
import type { TableData, RelationshipData } from "../components/ERDiagram/types";
 ...
 */
export function parseSqlToERData(sql: string): {
  tables: TableData[];
  ...
  let astList: any[];
  try {
    const result = sqlParser.astify(sql, { database: "MySQL" });
